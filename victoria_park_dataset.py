...
#
#         # Load data files based on the split
#         self.data_files = self._load_data_files()
#
#         if self.verbose:
#             print(f"Loaded {len(self.data_files)} data files from {data_root} for {split} split.")
#
#         # Preprocess data to generate sequences
#         self.sequences = self._preprocess_data()
#
#         if self.verbose:
#             print(f"Generated {len(self.sequences)} sequences for {split} split.")
#
...
#                 data_files.append(os.path.join(split_dir, file))
#
#         if not data_files:
#             raise ValueError(f"No .dat files found in {split_dir}.")
#
#         return data_files
#
#     def _preprocess_data(self):
#         """Preprocess data to generate sequences of point clouds."""
#         sequences = []
#
#         for file_path in self.data_files:
#             data = np.loadtxt(file_path)
#
#             # Ensure the data has enough rows to form a sequence
...
#                     # Extract laser scan (columns 6 to 6+361 are range values)
#                     ranges = data[i + j, 6:6 + 361]
#
#                     # Convert to 2D point cloud
#                     angles = np.linspace(-np.pi / 2, np.pi / 2, 361)
#                     x = ranges * np.cos(angles)
#                     y = ranges * np.sin(angles)
#                     points = np.stack([x, y, np.zeros_like(x)], axis=1)  # Add z=0
#
...
#         """Return a sequence of point clouds and dummy targets."""
#         sequence = self.sequences[idx]
#
#         # Convert point clouds to PyTorch tensors
#         point_clouds = [torch.from_numpy(pc).float() for pc in sequence]
#
#         # Since Victoria Park dataset has no ground truth, return dummy targets
#         targets = torch.zeros((self.sequence_length, 0))  # Placeholder for targets
#
#         return {
#             'pointclouds': point_clouds,
#             'targets': targets,
#             'metadata': {'sequence_id': idx}
#         }
#
#     @staticmethod
#     def collate_fn(batch):
#         """Custom collate function to handle sequences of point clouds."""
#         point_clouds = [item['pointclouds'] for item in batch]
#         targets = [item['targets'] for item in batch]
#         metadata = [item['metadata'] for item in batch]
#
#         # Stack point clouds and targets into batched tensors
#         point_clouds = torch.stack([torch.stack(pc) for pc in point_clouds], dim=0)
...
class VictoriaParkDataset(Dataset):
    ...
    def __init__(self, data_root, sequence_length=10, split='train', verbose=True):
        ...
        # 确保三个传感器数据长度一致
        min_length = min(len(self.laser_data), len(self.gps_data), len(self.drs_data))
        self.laser_data = self.laser_data[:min_length]
        self.gps_data = self.gps_data[:min_length]
        self.drs_data = self.drs_data[:min_length]

        if verbose:
            print(f"数据集加载完成:")
            print(f"  - 激光数据: {len(self.laser_data)} 帧")
            print(f"  - GPS数据: {len(self.gps_data)} 帧")
            print(f"  - DRS数据: {len(self.drs_data)} 帧")
            print(f"  - 序列长度: {sequence_length}")
            print(f"  - 可生成序列数: {max(0, len(self.laser_data) - sequence_length + 1)}")
